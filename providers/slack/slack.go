...
func (p *Provider) sendResource(ctx context.Context, msg uvim.OutboundMessage, ref uvim.ResourceRef) (result uvim.SendResult, err error) {
	...
	if !initResponse.OK || initResponse.UploadURL == "" || initResponse.FileID == "" {
		...
		return uvim.SendResult{}, uvim.NewProviderResponseErrorWithCode(initRaw, initResponse.Error, businessErr)
	}
	...
	if !completeResponse.OK {
		...
		return uvim.SendResult{}, uvim.NewProviderResponseErrorWithCode(responseRaw, completeResponse.Error, businessErr)
	}
...
func (p *Provider) resourceChannelID(ctx context.Context, target uvim.OutboundTarget) (channelID string, err error) {
	...
	if !response.OK || response.Channel.ID == "" {
		...
		return "", uvim.NewProviderResponseErrorWithCode(responseRaw, response.Error, businessErr)
	}
...
func ParseSendResponse(raw []byte) (string, error) {
	...
	if !response.OK {
		...
		return "", uvim.NewProviderResponseErrorWithCode(raw, response.Error, businessErr)
	}
